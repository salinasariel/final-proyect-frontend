...
const JobOffer = ({
    ...
    const changeStateOffer = async (offerId) => {
        try {
            const response = await api.post(`/OffersCotroller/ChangeStateOffer?ofertaId=${offerId}`);
            setOffers(response.data);
    ...
    const showEnterpriseData = async () => {
        try {
            ...
            const pending = response.data.filter(enterprise => !enterprise.userState);
            const authorized = response.data.filter(enterprise => enterprise.userState);
    ...
    const changeStateEnterprise = async (enterpriseId) => {
        try {
            const response = await api.put(`/User/ChangeStateEnterprise/${enterpriseId}`);
            setStudents(response.data);
    ...
    const filteredAuthorizedEnterprises = authorizedEnterprises.filter((enterprise) =>
        enterprise.name.toLowerCase().includes(searchTerm.toLowerCase())
    );
                            ...
                            <Table aria-label="simple table">
                                <TableHead className="bg-gray-100 dark:bg-gray-700">
                                    <TableRow>
                                        ...
                                        <TableCell className="dark:text-white">Nombre de oferta</TableCell>
                                ...
                                <TableBody>
                                    {filteredPendingOffers.map((offer) => (
                                        <TableRow key={offer.userId} className="dark:bg-gray-800">
                                            <TableCell className="dark:text-white">{offer.offerId}</TableCell>
                                            <TableCell className="dark:text-white">{offer.tittle}</TableCell>
                                            <TableCell className="dark:text-white">{offer.enterprise.name}</TableCell>
                                            <TableCell>
                                                <button onClick={() => changeStateOffer(offer.offerId)} className="text-blue-600 dark:text-blue-400 hover:underline">
                            ...
                            <Table aria-label="simple table">
                                <TableHead className="bg-gray-100 dark:bg-gray-700">
                                    <TableRow>
                                        ...
                                        <TableCell className="dark:text-white">Nombre de oferta</TableCell>
                                        <TableCell className="dark:text-white">Nombre de la empresa</TableCell>
                                        <TableCell className="dark:text-white">Desactivar</TableCell>
                                    </TableRow>
                                </TableHead>
                                <TableBody>
                            ...
                            <Table aria-label="simple table">
                                ...
                                <TableBody>
                                    {filteredPendingEnterprises.map((enterprise) => (
                                        <TableRow key={enterprise.userId} className="dark:bg-gray-800">
                                            <TableCell className="dark:text-white">{enterprise.userId}</TableCell>
                                            <TableCell className="dark:text-white">{enterprise.name}</TableCell>
                                            <TableCell className="dark:text-white">Pendiente</TableCell>
                                            <TableCell>
                                                <button onClick={() => changeStateEnterprise(enterprise.userId)} className="text-blue-600 dark:text-blue-400 hover:underline">
                                                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth="1.5" stroke="currentColor" className="size-6">
                                                        <path strokeLinecap="round" strokeLinejoin="round" d="M9 12.75 11.25 15 15 9.75M21 12a9 9 0 1 1-18 0 9 9 0 0 1 18 0Z" />
                                                    </svg>
                                                </button>
